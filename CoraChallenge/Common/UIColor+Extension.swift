//
//  UIColor+Extension.swift
//  CoraChallenge
//
//  Created by Fellipe Ricciardi Chiarello on 4/20/22.
//

import UIKit

extension UIColor {
    static let coraPink = UIColor(red: 254/255, green: 62/255, blue: 109/255, alpha: 1)
    static let coraLightGray = UIColor(red: 240/255, green: 244/255, blue: 248/255, alpha: 1)
    static let coraDarkGray = UIColor(red: 107/255, green: 112/255, blue: 118/255, alpha: 1)
    static let coraMediumGray = UIColor(red: 199/255, green: 203/255, blue: 207/255, alpha: 1)
    static let coraBackgroundWhite = UIColor(red: 229/255, green: 229/255, blue: 229/255, alpha: 1)
    static let coraBlack = UIColor(red: 59/255, green: 59/255, blue: 59/255, alpha: 1)
    static let coraBlue = UIColor(red: 26/255, green: 147/255, blue: 218/255, alpha: 1)
}
